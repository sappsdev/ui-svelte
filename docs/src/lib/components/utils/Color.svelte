...
<div class="overflow-y-auto p-8">
	<div class="flex flex-col gap-3 min-w-max">
		...
		{#each Object.keys(colors) as colorName}
			<div class="flex items-center mb-0 gap-3">
				<div class="w-24 text-xs font-medium">
					{colorName}
				</div>
				{#each shades as shade}
					...
					<button
						onclick={() => handleClick(colorName, shade)}
						class="size-8 shadow shadow-muted rounded-md"
						style="background-color: {colors[colorName][shade]};"
					/>
				{/each}
			</div>
		{/each}
